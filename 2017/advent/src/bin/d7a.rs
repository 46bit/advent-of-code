...
fn d7a(input: &str) -> String {
    ...
    let mut disc_name = discs[&root_name.clone().unwrap()].name.clone();
    while !discs[&disc_name].child_names.is_empty() {
        disc_name = discs[&disc_name].child_names.last().unwrap().clone();
    }
    println!(
        "root={:?} bottom={:?}",
        root_name.clone().unwrap(),
        disc_name
    );
    return disc_name;
}
